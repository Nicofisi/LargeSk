...
public class Register {
  ...
  public void registerAll() {
    ...
    if (isPluginOnServer("AAC") && config.getConfigurationSection("enable").getBoolean("AAC")) {
      Xlog.logInfo(
          "I have found AAC on your server. I'm pleased to announce we will work together from now.");
    ...
    // SkinsRestorer
    if (isPluginOnServer("SkinsRestorer")
        && config.getConfigurationSection("enable").getBoolean("SkinsRestorer")) {
      Xlog.logInfo(
          "SkinsRestorer has been detected! Registring conditions, expressions and other boring stuff..");
