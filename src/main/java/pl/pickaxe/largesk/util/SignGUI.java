...
public class SignGUI {
    ...
    ProtocolLibrary.getProtocolManager().addPacketListener(
        packetListener = new PacketAdapter(plugin, PacketType.Play.Client.UPDATE_SIGN) {
          ...
          public void onPacketReceiving(PacketEvent event) {
            ...
            final WrappedChatComponent[] chatarray =
                event.getPacket().getChatComponentArrays().getValues().get(0);
            final String[] lines = {chatarray[0].getJson(), chatarray[1].getJson(),
                chatarray[2].getJson(), chatarray[3].getJson()};
  ...
  public void open(Player player, String[] defaultText, SignGUIListener response) {
    ...
    PacketContainer packet133 =
        protocolManager.createPacket(PacketType.Play.Server.OPEN_SIGN_ENTITY);
